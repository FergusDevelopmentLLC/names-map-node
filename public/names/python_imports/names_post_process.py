...
def create_name_table ():
    delete = """drop table if exists public.name"""
    print (delete)
    cursor = database.cursor()
    mydata = cursor.execute(delete)
    cursor.execute("""create table public.name
                      (
                        name varchar(255),
                        sex varchar(2),
                        data text
                      ...
                      ALTER TABLE public.name OWNER TO geodevdb;""")
...
def create_piped_data ( name, sex ):
    delim = "|"
    piped_data = "name,sex,data"


    url = 'http://127.0.0.1:8645/popular_name/getOccurrences/' + str(name) + '/' + str(sex);
    print(url)
    ...
    results = open("/tmp/name_import.txt", "w")
    results.write(piped_data)
    results.close()

def copy_piped_data_to_name_table ():

    cursor = database.cursor()

    cursor.execute("""copy name from '/tmp/name_import.txt' DELIMITER '|' CSV HEADER;""")
    cursor.execute("""update name set data = replace(data, '''', '"' );""")
    cursor.execute("""update name set data = replace(data, ' ', '' );""")
    cursor.execute("""update name set data = replace(data, 'u"', '"' );""")
...
females = ["Mary","Patricia","Elizabeth","Jennifer","Linda","Barbara","Margaret"]
